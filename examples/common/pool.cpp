#include <process/MidasThreadPool.h>

void first(int id) { std::cout << "hello from " << id << ", function first\n"; }

void aga(int id, int par) { std::cout << "hello from " << id << ", function aga with parameter " << par << '\n'; }
...
struct Second {
    ...
    void operator()(int id) const { std::cout << "Second class hello from " << id << ' ' << this->s << '\n'; }
...
void ugu(int id, Third &t) {
    ...
    std::cout << "hello from " << id << ", function ugu with parameter Third " << t.v << '\n';
}
...
int main(int argc, char **argv) {
...

    {
        ...
        p.push(Second(", Second move functor"));     // functor, move ctor
    }
    ...
    std::cout << "thread 0 id is " << th.get_id() << '\n';
    return 0;
}
