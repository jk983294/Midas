...
TradeSession::TradeSession(const string& str) {
    ...
    if (itr1 != string::npos) {
        ...
        MIDAS_LOG_ERROR("trade sess ion init error: " << str);
    }
}
...
void TradeStatusManager::load_trade_session(const string& path) {
    if (midas::check_file_exists(path.c_str())) {
        ...
        if (ifs.is_open()) {
            ...
            while (getline(ifs, s)) {
                if (!s.empty()) {
                    auto itr1 = s.find_first_of(' ');
                    string productType = s.substr(0, itr1);
                    auto itr2 = itr1;
                    while ((itr2 = s.find_first_of(',', itr1 + 1)) != string::npos) {
                        product2sessions[productType].add_session({s.substr(itr1 + 1, itr2 - itr1 - 1)});
                        itr1 = itr2;
                    }
                    product2sessions[productType].add_session({s.substr(itr1 + 1)});
                }
            }
