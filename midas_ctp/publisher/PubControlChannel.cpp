                    ...
                    for (auto c = 0; c < hdr.count; ++c) {
                        switch (*pData) {
                            case CTRL_CONNECT_TYPE:
                                MIDAS_LOG_INFO("Dispatching on_connect");
                                publisher->on_connect(sockFd, &hdr, (CtrlConnect*)pData);
                                break;
                            case CTRL_DISCONNECT_TYPE:
                                MIDAS_LOG_INFO("Dispatching on_disconnect");
                                publisher->on_disconnect(sockFd, &hdr, (CtrlDisconnect*)pData);
                                break;
                            case CTRL_SUBSCRIBE_TYPE:
                                publisher->on_subscribe(sockFd, &hdr, (CtrlSubscribe*)pData);
                                break;
                            case CTRL_UNSUBSCRIBE_TYPE:
                                publisher->on_unsubscribe(sockFd, &hdr, (CtrlUnsubscribe*)pData);
                                break;
                            default:
                                break;
                        }
    ...
    while (!stopped) {
        ...
        for (int n = 0; n < nfds; ++n) {
            ...
            if (fdReady == fdControl) {
                ...
                if (fdClient >= 0) {
                    MIDAS_LOG_INFO("Accepted connection(" << fdClient << ") from consumer at " << ipClient << ":"
                                                          << portClient);
                    publisher->register_client(fdClient, ipClient, portClient);
                    if (ipClient) {
                        free(ipClient);
                    }
                ...
                if (packetCount < 0) {  // Socket would have been closed by receive and removed from epoll
                    ...
                    publisher->unregister_client(fdReady);
                } else {
